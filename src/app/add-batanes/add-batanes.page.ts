...
@Component({
  selector: 'app-add-batanes',
  templateUrl: './add-batanes.page.html',
  styleUrls: ['./add-batanes.page.scss'],
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
...
export class AddBatanesPage implements OnInit {
  // Step 1: File Info
  fileName: string = '';
  commodity: string = '';
  ...
  async submitForm() {
    ...
    try {
      ...
      await this.router.navigateByUrl('/batanes', { replaceUrl: true });
    } catch (error) {
      await loading.dismiss();
